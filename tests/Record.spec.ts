...
describe('Record', () => {
  ...
  beforeEach(() => {
    rawAlbum = {
      ...
      band: {
        ...
        name: 'Genesis',
        members: [
          {id: 1, firstName: 'Peter', lastName: 'Gabriel', birthDate: '1950-02-13'},
          {id: 2, firstName: 'Banks', lastName: 'Tony', birthDate: '1950-03-27'},
          {id: 3, firstName: 'Rutherford', lastName: 'Mike', birthDate: '1950-10-02'},
          {id: 4, firstName: 'Hackett', lastName: 'Steve', birthDate: '1950-02-12'},
          {id: 5, firstName: 'Collins', lastName: 'Phil', birthDate: '1951-01-30'}
        ]
      },
  ...
  describe('pseudo-refelection methods', () => {
    it('# get attributesNames', () => {
      ...
      albumAttributeNames.forEach(n => expect(album.attributesNames).toContain(n))

      const track = new Track()
      const trackAttributeNames = ['id', 'name', 'duration']
      expect(track.attributesNames.length).toEqual(trackAttributeNames.length)
      trackAttributeNames.forEach(n => expect(track.attributesNames).toContain(n))
    })
    ...
    it('# get associationsNames', () => {
      ...
      expect(album.associationsNames.length).toEqual(albumAssociationsNames.length)
      albumAssociationsNames.forEach(n => expect(album.associationsNames).toContain(n))
    })
